...
package com.serialportapi.usbdriver_api;
...
public class FtdiSerialDriver extends CommonUsbSerialDriver {
     ...
     * @param dest           The destination buffer to write the status bytes into (can be
     *                       src)
     ...
     * @throws UsbSerialRuntimeException if the given device is incompatible with this driver
     */
    ...
    public boolean purgeHwBuffers(boolean purgeReadBuffers,
        ...
        if (purgeReadBuffers) {
            int result = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE,
                    SIO_RESET_REQUEST, SIO_RESET_PURGE_RX, 0 /* index */, null,
        ...
        if (purgeWriteBuffers) {
            int result = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE,
                    SIO_RESET_REQUEST, SIO_RESET_PURGE_TX, 0 /* index */, null,
                    0, USB_WRITE_TIMEOUT_MILLIS);
            if (result != 0) {
                throw new IOException("Flushing RX failed: result=" + result);
            }
    ...
    public static Map<Integer, int[]> getSupportedDevices() {
        ...
        supportedDevices.put(Integer.valueOf(UsbId.VENDOR_FTDI), new int[]{
                UsbId.FTDI_FT232R, UsbId.FTDI_FT231X,});
